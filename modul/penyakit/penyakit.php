...
if (!(isset($_SESSION['username']) && isset($_SESSION['password']))) {
  ...
  switch ($_GET['act']) {
    ...
    default:
      ...
      $tampil = mysqli_query($conn,"SELECT * FROM penyakit ORDER BY nama_penyakit");
      echo "<form method=POST action='?module=penyakit' name=text_form onsubmit='return Blank_TextField_Validator_Cari()'>
          <br><br></form>";
      $baris = mysqli_num_rows($tampil);
      if ($_POST['Go']) {
        $numrows = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM penyakit where nama_penyakit like '%$_POST[keyword]%' ORDER BY nama_penyakit"));
        if ($numrows > 0) {
		  ...
		  <tbody>";
          $hasil = mysqli_query($conn,"SELECT * FROM penyakit where nama_penyakit like '%$_POST[keyword]%' ORDER BY nama_penyakit");
          $no = 1;
          $counter = 1;
        ...
        if ($baris > 0) {
		  ...
		  <tbody>
		  ";
          $hasil = mysqli_query($conn,"SELECT * FROM penyakit ORDER BY nama_penyakit limit $offset,$limit");
          $no = 1;
          $no = 1 + $offset;
          $counter = 1;
          while ($r = mysqli_fetch_array($hasil)) {
